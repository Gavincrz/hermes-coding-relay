...
- 继续观察真实使用中是否还会出现绕过 relay 的异常案例；若出现，再单独立任务定位
...
- 保持观察；若后续再发现 Hermes 存在隐式 resume 倾向，再单独补约束或修正 skill
...
- 若后续调整 resume 提示模板，再补真实平台顺序回归验证
...
- 已由真实飞书联调覆盖；若后续再次出现上下文缺口，再临时加诊断并单独追踪
...
- 保持当前约束；若 Hermes 再次回退到外部会话源搜索，再单独立任务修复
...
- 当前飞书联调未见回退；若后续需要再次排障，再按需临时开启诊断
...
- 若后续继续推进 provider-neutral 命名收口，再单独立任务整理内部 runtime 和事件层命名

---

## T025 Codex v1 收口与下一阶段入口

状态：done

目标：

- 将“飞书端到端已验证”这一阶段性结论写回任务表
- 明确当前 Codex-only v1 主链路已闭环
- 把后续开发入口收束到下一阶段任务，避免继续围绕已完成的飞书验证反复追加尾项

完成标准：

- 任务表中与飞书真实验证直接相关的尾项已收口，不再把它们错误保留为待办
- 明确当前阶段可视为 Codex-first relay v1 已闭环
- 明确后续若继续扩展，正式入口为 `T011A OpenCode 调研与接入边界`

实现备注：

- 已完成：收口 `T012`、`T015`、`T017`、`T018`、`T019`、`T020`、`T021`、`T022`、`T023`、`T024` 中仍残留的“待真实飞书验证”尾项
- 已完成：保留尚未解决的真实产品边界，例如其他 gateway 平台未验证、命令高价值规则尚未配置化等，但不再与本轮飞书验证混写
- 已完成：将下一阶段入口明确回收到 `T011A`，避免在 Codex v1 已闭环后继续以零散尾项推进

未完成内容：

- `T011 OpenCode 支持` 仍未开始；这属于下一阶段扩展，不属于当前 Codex-only v1 收口范围

下一步：

- 若确认继续扩展多后端支持，按顺序进入 `T011A OpenCode 调研与接入边界`
